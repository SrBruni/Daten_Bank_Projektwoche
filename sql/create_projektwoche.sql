...
CREATE TABLE schuelerInnen(
	...
	sJahrgangsstufe INTEGER		CHECK (sJahrgangsstufe BETWEEN 7 AND 10),
	sProfilnummer 	INTEGER  	CHECK (sProfilnummer BETWEEN 1 AND 3),
	sKlassennummer  INTEGER 	CHECK (sKlassennummer BETWEEN 0 AND 3)
);
...
CREATE TABLE projekt(
	...
	pMaxPers		INTEGER	CHECK (pMaxPers BETWEEN 5 AND 70)	not null,
	pBeschreibung	VARCHAR(1000)
);
...
CREATE TABLE leitet_mit (
	Mitleitungen	VARCHAR			REFERENCES angestellte(aKuerzel)	not null,
	pNummer			INTEGER 		REFERENCES projekt(pNummer)			not null,
	PRIMARY KEY (Mitleitungen, pNummer)
);
...
CREATE TABLE findet_statt(
	...
	datum			DATE		CHECK (datum BETWEEN '2024-06-23' AND '2024-06-30'),
	von_tageszeit 	TIME		DEFAULT '09:00:00', 
	bis_tageszeit 	TIME		DEFAULT '15:00:00', 
	ort 			VARCHAR	 	REFERENCES raum_oder_ort(oBezeichnung)	not null,
